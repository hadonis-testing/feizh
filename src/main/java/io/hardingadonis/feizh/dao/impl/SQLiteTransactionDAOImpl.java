  ...
  public List<Transaction> getAll() {
    ...
    try {
      PreparedStatement smt =
          conn.prepareStatement("SELECT * FROM `transaction` ORDER BY `id` DESC");
    ...
    try {
      PreparedStatement smt =
          conn.prepareStatement(
              "INSERT INTO `transaction`(`source_wallet_id`, `amount`, `description`, `type`,"
                  + " `create_at`) VALUES(?, ?, ?, ?, ?)");
    ...
    try {
      ...
      switch (duration) {
        case "today":
          {
            sql =
                "SELECT SUBSTR(`create_at`, 1, 10) AS `date`, SUM(`amount`) AS `total`  FROM"
                    + " `transaction` WHERE `type` = ? AND `date` = DATE('now') GROUP BY `date`;";
            break;
          }
        ...
        case "yesterday":
          {
            sql =
                "SELECT SUBSTR(`create_at`, 1, 10) AS `date`, SUM(`amount`) AS `total`  FROM"
                    + " `transaction` WHERE `type` = ? AND `date` = DATE('now', '-1 days') GROUP BY"
                    + " `date`;";
            break;
          }
        ...
        case "1week":
          {
            sql =
                "SELECT SUBSTR(`create_at`, 1, 10) AS `date`, SUM(`amount`) AS `total`  FROM"
                    + " `transaction` WHERE `type` = ? AND `date` >= DATE('now', '-6 days') GROUP"
        ...
        case "1month":
          {
            sql =
                "SELECT SUBSTR(`create_at`, 1, 10) AS `date`, SUM(`amount`) AS `total`  FROM"
                    + " `transaction` WHERE `type` = ? AND `date` >= DATE('now', '-1 month') GROUP"
                    + " BY `date` ORDER BY `date`;";
            break;
          }
        ...
        case "3months":
          {
            sql =
                "SELECT SUBSTR(`create_at`, 1, 7) AS `date`, SUM(`amount`) AS `total`  FROM"
                    + " `transaction` WHERE `type` = ? AND `date` >= DATE('now', '-3 months') GROUP"
                    + " BY `date` ORDER BY `date`;";
            break;
          }
        ...
        case "6months":
          {
            sql =
                "SELECT SUBSTR(`create_at`, 1, 7) AS `date`, SUM(`amount`) AS `total`  FROM"
                    + " `transaction` WHERE `type` = ? AND `date` >= DATE('now', '-6 months') GROUP"
                    + " BY `date` ORDER BY `date`;";
            break;
          }
        ...
        case "1year":
          {
            sql =
                "SELECT SUBSTR(`create_at`, 1, 7) AS `date`, SUM(`amount`) AS `total`  FROM"
                    + " `transaction` WHERE `type` = ? AND `date` >= DATE('now', '-1 year') GROUP"
                    + " BY `date` ORDER BY `date`;";
            break;
          }
      }
    ...
    try {
      ...
      switch (duration) {
        case "today":
          {
            sql =
                "SELECT `w`.`name`, SUM(`t`.`amount`) AS `total` FROM `transaction` `t` JOIN"
                    + " `wallet` `w` ON `t`.`source_wallet_id` = `w`.`id` WHERE `t`.`type` = ? AND"
                    + " SUBSTR(`t`.`create_at`, 1, 10) = DATE('now') GROUP BY `w`.`id`;";
            break;
          }
        ...
        case "yesterday":
          {
            sql =
                "SELECT `w`.`name`, SUM(`t`.`amount`) AS `total` FROM `transaction` `t` JOIN"
                    + " `wallet` `w` ON `t`.`source_wallet_id` = `w`.`id` WHERE `t`.`type` = ? AND"
                    + " SUBSTR(`t`.`create_at`, 1, 10) = DATE('now', '-1 days') GROUP BY `w`.`id`;";
            break;
          }
        ...
        case "1week":
          {
            sql =
                "SELECT `w`.`name`, SUM(`t`.`amount`) AS `total` FROM `transaction` `t` JOIN"
                    + " `wallet` `w` ON `t`.`source_wallet_id` = `w`.`id` WHERE `t`.`type` = ? AND"
                    + " SUBSTR(`t`.`create_at`, 1, 10) >= DATE('now', '-6 days') GROUP BY"
                    + " `w`.`id`;";
            break;
          }
        ...
        case "1month":
          {
            sql =
                "SELECT `w`.`name`, SUM(`t`.`amount`) AS `total` FROM `transaction` `t` JOIN"
                    + " `wallet` `w` ON `t`.`source_wallet_id` = `w`.`id` WHERE `t`.`type` = ? AND"
                    + " SUBSTR(`t`.`create_at`, 1, 7) >= DATE('now', '-1 month') GROUP BY"
                    + " `w`.`id`;";
            break;
          }
        ...
        case "3months":
          {
            sql =
                "SELECT `w`.`name`, SUM(`t`.`amount`) AS `total` FROM `transaction` `t` JOIN"
                    + " `wallet` `w` ON `t`.`source_wallet_id` = `w`.`id` WHERE `t`.`type` = ? AND"
                    + " SUBSTR(`t`.`create_at`, 1, 7) >= DATE('now', '-3 months') GROUP BY"
                    + " `w`.`id`;";
            break;
          }
        ...
        case "6months":
          {
            sql =
                "SELECT `w`.`name`, SUM(`t`.`amount`) AS `total` FROM `transaction` `t` JOIN"
                    + " `wallet` `w` ON `t`.`source_wallet_id` = `w`.`id` WHERE `t`.`type` = ? AND"
                    + " SUBSTR(`t`.`create_at`, 1, 7) >= DATE('now', '-6 months') GROUP BY"
                    + " `w`.`id`;";
            break;
          }
        ...
        case "1year":
          {
            sql =
                "SELECT `w`.`name`, SUM(`t`.`amount`) AS `total` FROM `transaction` `t` JOIN"
                    + " `wallet` `w` ON `t`.`source_wallet_id` = `w`.`id` WHERE `t`.`type` = ? AND"
                    + " SUBSTR(`t`.`create_at`, 1, 7) >= DATE('now', '-1 year') GROUP BY `w`.`id`;";
            break;
          }
      }
